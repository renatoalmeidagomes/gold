...
export default function Footer() {
  ...
  const addressLines = [config.addressLine1, config.addressLine2, config.addressLine3].filter(Boolean);
    ...
    <footer id="contato" className="bg-brand-black pt-16 md:pt-20 pb-24 md:pb-10 border-t border-white/5">
      <div className="container mx-auto px-4 md:px-6 text-center md:text-left">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-12">
          <div>
            <span className="font-heading font-black text-2xl tracking-widest uppercase block mb-4 text-gradient-gold">
              {config.storeName || 'Ecommerce'}
            </span>
            ...
            <div className="flex justify-center md:justify-start gap-4">
              <a href={instagramUrl} target="_blank" className="w-10 h-10 rounded-full border border-white/10 flex items-center justify-center hover:border-brand-gold hover:text-brand-gold transition-colors text-lg">
          ...
          <div className="flex flex-col items-center md:items-start space-y-4">
            ...
            <p className="text-white text-xs uppercase font-black tracking-widest">{config.storeName || 'Ecommerce'}</p>
            {addressLines.length > 0 ? (
              addressLines.map((line, index) => (
                <p key={index} className="text-gray-400 text-xs uppercase font-bold tracking-widest flex items-center gap-2">
                  {index === 0 && <i className="fa-solid fa-location-dot text-brand-gold"></i>}
                  {index > 0 && <span className="w-[14px]"></span>}
                  {line}
                </p>
              ))
            ) : (
        ...
        <div className="pt-8 border-t border-white/5 text-center">
          <p className="text-gray-600 text-[10px] md:text-xs font-bold uppercase tracking-[0.2em]">
            &copy; {new Date().getFullYear()} {config.storeName || 'Ecommerce'}. Todos os direitos reservados.
          </p>
        </div>
      </div>
    </footer>
